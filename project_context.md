...
Current phase: P15 donor profiles, guest fields, and privacy-safe identity handling complete; P16 is next
Public production alias (unchanged): https://church-tithing-platform-3k2j.vercel.app/
Latest non-production preview branch: https://github.com/abdulnafa/church-tithing-platform/tree/codex/p15-preview-20260909
...
- `[PENDING]` P16 is the next roadmap task: store prayer requests separately from accounting data with explicit consent, restricted access, retention rules, and audit coverage.
...
- `[COMPLETE]` Tenant settings, funds, campaigns, staff, platform management, public giving configuration, and linked donor profiles now use persisted development data. Remaining dashboard financial figures are explicitly labelled shared examples; live payments and production email delivery are not enabled.
...
- `[COMPLETE]` The public giving UI uses tenant-scoped persisted church, fund, and campaign configuration, supports target selection, amount entry, and one-time/weekly/monthly intent, and locally validates required guest name/email fields without sending, saving, looking up, or linking them. It does not request message, prayer-request, card, bank, or other payment information.
- `[COMPLETE]` Online payment submission remains disabled until the approved consent and payment flows are implemented in P16 and P18.
...
`P16 - Prayer requests and consent separation` is `[PENDING]`. Store prayer requests separately from accounting and receipt data, require explicit consent, restrict staff access, define retention behavior without inventing client policy, and keep live payment submission disabled until the later giving flow is approved and implemented.
