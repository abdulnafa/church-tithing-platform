...
Current phase: P15 donor profiles, guest fields, and privacy-safe identity handling in progress
...
- `[IN PROGRESS]` P15 - Implement donor profiles, required guest fields, account updates, and the approved guest-to-member linking policy.
...
`P15 - Donor profiles, guest fields, and guest-to-member linking` is `[IN PROGRESS]`. Implement the minimum donor identity fields, authenticated profile updates, guest-giving requirements, and the conservative privacy-preserving guest/member boundary without enabling payment submission prematurely. Historical guest financial records must not be automatically claimed by matching an email address while the detailed client linking policy remains pending.
