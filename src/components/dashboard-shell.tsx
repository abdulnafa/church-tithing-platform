...
export function DashboardShell({ children, kind, title, subtitle }: DashboardShellProps) {
  ...
  const showDemoSwitch = process.env.NODE_ENV !== "production" || process.env.NEXT_PUBLIC_DEMO_MODE === "true";
  ...
  return (
    <div className="min-h-screen bg-[#f3f1eb] lg:grid lg:grid-cols-[244px_1fr]">
      <aside className="hidden min-h-screen flex-col bg-[var(--ink)] px-4 py-5 text-white lg:flex">
        ...
        {!isPlatform && (
          <div className="mt-7 flex items-center gap-3 rounded-2xl bg-white/[0.07] p-3">
            <ChurchMark size="sm" />
            <div className="min-w-0"><p className="truncate text-xs font-bold">Harbour Grace</p><p className="mt-1 text-[9px] uppercase tracking-wider text-white/45">{isMember ? "My church" : "Church workspace"}</p></div>
          </div>
        )}
        ...
        <nav className="mt-7 space-y-1">
          {navByKind[kind].map(([label, href, Icon], index) => (
            <Link className={`focus-ring flex items-center gap-3 rounded-xl px-3 py-3 text-xs font-semibold transition ${index === 0 ? "bg-white !text-[#122235]" : "text-white/58 hover:bg-white/[0.07] hover:text-white"}`} href={href} key={label}>
              <Icon size={18} /> {label}
            </Link>
          ))}
        </nav>
        <div className="mt-auto border-t border-white/10 pt-4">
          <div className="flex items-center gap-3 px-2 py-2"><span className="grid size-9 place-items-center rounded-full bg-[var(--sage)] text-xs font-bold">{userName.split(" ").map((word) => word[0]).join("")}</span><div className="min-w-0"><p className="truncate text-xs font-bold">{userName}</p><p className="mt-0.5 text-[9px] text-white/45">{role}</p></div></div>
          <Link className="mt-2 block rounded-lg px-2 py-2 text-[10px] font-semibold text-white/45 hover:text-white" href="/">← Exit demo</Link>
        </div>
      </aside>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-30 border-b border-[var(--line)] bg-[#f3f1eb]/90 backdrop-blur-xl">
          <div className="flex h-17 items-center justify-between px-4 sm:px-7 lg:px-9">
            ...
            <div className="flex items-center gap-2">
              {showDemoSwitch && <Link className="focus-ring hidden rounded-full border border-[var(--line)] bg-white px-4 py-2 text-[10px] font-bold text-[var(--ink-soft)] sm:block" href={kind === "member" ? "/church" : kind === "church" ? "/platform" : "/dashboard"}>Switch demo role</Link>}
              <button aria-label="Notifications" className="focus-ring relative grid size-10 place-items-center rounded-full border border-[var(--line)] bg-white text-[var(--ink-soft)]"><BellIcon size={18} /><span className="absolute right-2 top-2 size-1.5 rounded-full bg-[var(--coral)]" /></button>
        ...
        <nav className="fixed inset-x-0 bottom-0 z-40 flex border-t border-[var(--line)] bg-white px-2 pb-[max(.5rem,env(safe-area-inset-bottom))] pt-2 lg:hidden">
          {navByKind[kind].slice(0, 4).map(([label, href, Icon], index) => <Link className={`flex flex-1 flex-col items-center gap-1 rounded-xl py-1.5 text-[9px] font-semibold ${index === 0 ? "text-[var(--sage)]" : "text-[var(--muted)]"}`} href={href} key={label}><Icon size={18} /><span className="max-w-20 truncate">{label}</span></Link>)}
        </nav>
      </div>
    </div>
  );
...
export function StatCard({ label, value, note, icon, tone = "sage" }: { label: string; value: string; note: string; icon: ReactNode; tone?: "sage" | "gold" | "blue" | "coral" }) {
  ...
  return <article className="soft-card rounded-[20px] p-5"><div className="flex items-start justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-[var(--muted)]">{label}</p><p className="mt-3 text-2xl font-bold tracking-[-0.04em]">{value}</p></div><span className={`grid size-10 place-items-center rounded-2xl ${colors[tone]}`}>{icon}</span></div><p className="mt-3 text-[10px] text-[var(--muted)]">{note}</p></article>;
}

export function SectionHeader({ title, eyebrow, action }: { title: string; eyebrow?: string; action?: ReactNode }) {
  return <div className="flex items-end justify-between gap-4"><div>{eyebrow && <p className="text-[9px] font-bold uppercase tracking-[0.16em] text-[var(--sage)]">{eyebrow}</p>}<h2 className="mt-1 text-base font-bold tracking-tight">{title}</h2></div>{action}</div>;
}
