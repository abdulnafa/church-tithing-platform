...
export default function MemberDashboardPage() {
  ...
  return (
    <DashboardShell kind="member" subtitle="Your generosity, all in one place" title="My giving">
      <div className="mx-auto max-w-6xl pb-24">
        <div className="mb-7 lg:hidden"><p className="text-xs text-[var(--muted)]">Good morning, Alicia</p><h1 className="font-display mt-1 text-3xl tracking-[-0.035em]">Your giving</h1></div>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard icon={<DownloadIcon size={19} />} label="2025 statement" note="Published 12 Jan 2026" tone="coral" value="Ready" />
        </div>

        <div className="mt-6 grid gap-6 xl:grid-cols-[1.18fr_.82fr]">
          <section className="soft-card rounded-[22px] p-5 sm:p-6" id="history">
            <SectionHeader action={<button className="text-[10px] font-bold text-[var(--sage)]">View all</button>} eyebrow="Recent activity" title="Giving history" />
          ...
          <div className="space-y-6">
            <section className="overflow-hidden rounded-[22px] bg-[var(--ink)] p-5 text-white sm:p-6" id="recurring">
              ...
              <div className="mt-5 grid grid-cols-2 gap-2"><button className="rounded-full bg-white px-4 py-2.5 text-[10px] font-bold text-[var(--ink)]">Manage gift</button><button className="rounded-full border border-white/15 px-4 py-2.5 text-[10px] font-bold text-white/75">Pause</button></div>
            </section>

            <section className="soft-card rounded-[22px] p-5" id="payments">
              <SectionHeader title="Payment method" />
              <div className="mt-4 flex items-center gap-3 rounded-2xl bg-[#f0eee8] p-4"><span className="grid size-10 place-items-center rounded-xl bg-white text-[var(--sage)]"><CardIcon size={18} /></span><div className="min-w-0 flex-1"><p className="text-xs font-bold">Visa ending 4242</p><p className="mt-1 text-[9px] text-[var(--muted)]">Default · Expires 09/2029</p></div><button className="text-[9px] font-bold text-[var(--sage)]">Manage</button></div>
            </section>

            <section className="soft-card rounded-[22px] p-5" id="statements">
              <SectionHeader title="Annual statement" />
              <p className="mt-3 text-xs leading-5 text-[var(--muted)]">Your 2025 giving statement is ready to download.</p>
              <button className="focus-ring mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full border border-[var(--line)] bg-white px-4 py-3 text-[10px] font-bold"><DownloadIcon size={15} /> Download PDF</button>
            </section>
          </div>
        </div>

        <section className="soft-card mt-6 flex flex-col gap-4 rounded-[22px] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6" id="profile"><div><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-[var(--sage)]">Personal details</p><p className="mt-2 text-sm font-bold">Alicia Clarke</p><p className="mt-1 text-[10px] text-[var(--muted)]">alicia.clarke@example.com · +1 (246) 555-0178</p></div><button className="focus-ring rounded-full border border-[var(--line)] bg-white px-4 py-2.5 text-[10px] font-bold">Edit profile</button></section>

        <Link className="mt-6 flex items-center justify-between rounded-[22px] bg-[var(--sage)] p-5 text-white sm:p-6" href="/give/harbour-grace"><div><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-white/55">Make an impact</p><p className="mt-2 text-base font-bold">Give again to Harbour Grace</p></div><span className="grid size-10 place-items-center rounded-full bg-white !text-[#155247]"><ArrowRightIcon size={18} /></span></Link>
      </div>
    </DashboardShell>
  );
}
